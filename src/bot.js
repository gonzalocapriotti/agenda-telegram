...
bot.command('borrar', (ctx) => {
  ...
  const id = partes[1];
  if (!id) return ctx.reply("⚠️ Indicá el ID. Ejemplo: `/borrar 2`", { parse_mode: 'Markdown' });
  ...
  db.run("DELETE FROM recordatorios WHERE id = ? AND user_id = ?", [id, ctx.from.id], function(err) {
    ...
    ctx.reply(`🗑️ Recordatorio ID ${id} eliminado.`);
  });
});
...
bot.on(['text', 'voice'], async (ctx) => {
  ...
  try {
    if (ctx.message.voice) {
      ...
      if (!textoUsuario) {
        return ctx.reply("⚠️ No pude interpretar el audio.");
      }
